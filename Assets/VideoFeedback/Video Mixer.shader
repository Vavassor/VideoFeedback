    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            inline float4 detectEdges(sampler2D samp, float2 uv)
            {
                ...
                float n0 = dot(tex2D(samp, uv + offset.yz).rgb, luminanceConv);
                float s0 = dot(tex2D(samp, uv - offset.yz).rgb, luminanceConv);
                float e0 = dot(tex2D(samp, uv + offset.xz).rgb, luminanceConv);
                float w0 = dot(tex2D(samp, uv - offset.xz).rgb, luminanceConv);
                float ne0 = dot(tex2D(samp, uv + offset.xy).rgb, luminanceConv);
                float nw0 = dot(tex2D(samp, uv + offset.wy).rgb, luminanceConv);
                float se0 = dot(tex2D(samp, uv - offset.wy).rgb, luminanceConv);
                float sw0 = dot(tex2D(samp, uv - offset.xy).rgb, luminanceConv);
            ...
            fixed4 frag(v2f_customrendertexture IN) : COLOR
            {
                ...
                float4 edges = detectEdges(_MainTex, distortedTexcoord);
                float4 currentColor;
